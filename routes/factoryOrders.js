...
async function enrichOrders(orders) {
  ...
  return orders.map((order) => {
    ...
    const brandProfile = brandMap.get(order.brand?.toString?.() ?? order.brand);
    if (brandProfile) {
      obj.brandName = brandProfile.brandName || brandProfile.name || obj.brandName;
      obj.brandId = brandProfile.userId?.toString?.() ?? order.brand;
    }
    return obj;
  });
}
...
function buildStatusOptions(status) {
  switch (status) {
    ...
    case 'pending_payment':
      return ['in_progress', 'waiting_delivery', 'out_for_delivery'];
    case 'accepted':
      return ['in_progress', 'waiting_delivery', 'out_for_delivery'];
    case 'in_progress':
      return ['waiting_delivery', 'out_for_delivery'];
...
router.put('/orders/:id/status', auth, async (req, res) => {
  try {
    ...
    await Notification.create({
      user: order.brand,
      title: status === 'accepted' ? 'Order Accepted!' : status === 'rejected' ? 'Order Update' : 'Order Status Updated',
